      ...
      <a-layout-header theme="light" style="border-bottom: 1px solid rgb(218 218 218);height: auto;background-color:#fff;">
        <div style="display: flex; padding: 5px 0px; justify-content: space-between;">
          ...
          <a-space>
            ...
            <a-button @click="showXML">
              <i class="iconfont icon-browse" />XML
            </a-button>
            <a-button @click="saveXML(true)" >
              <i class="iconfont icon-download" />XML
            </a-button>
            <a-button @click="saveImg('svg', true)">
              <i class="iconfont icon-download" />SVG
            </a-button>
            <a-button type="primary" @click="save" v-if="isEdit">
              <i class="iconfont icon-upload" /> 保存
            </a-button>
...
export default {
  ...
  methods: {
    ...
    fillColor () {
      ...
      console.log(canvas.addMarker('task1564480905805', 'highlight'))
      this.modeler._definitions.rootElements[0].flowElements.forEach(n => {
        if (n.$type === 'bpmn:SequenceFlow') {
          canvas.addMarker(n.id, 'highlight-line')
        }
